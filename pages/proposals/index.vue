            ...
            if (!proposal.status) {
              ...
              if (proposal.state === 0) {
                if (!proposal.cycle) {
                  ...
                  if (proposalCycle && proposal.cycle === this.$dao.cycleConfig.current_cycle && this.$moment(proposalCycle.start_time + 'Z').add(this.$dao.proposalConfig.cycle_voting_duration_sec, 'seconds').isAfter()) {
                    status = 'ACTIVE'
