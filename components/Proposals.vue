          ...
          <div class="column is-one-fifths-desktop has-text-left-mobile has-text-right-desktop">
            ...
            <div class="is-size-7" v-if="(proposal.status ==='ACTIVE' || proposal.status ==='CLOSED')">
               <span v-for="result in proposal.vote_counts" :key="result.key" class="vote-result">
                <small><b :class="{'has-text-success': result.key === 1, 'has-text-danger': result.key === 2}" :data-tooltip="voteTypes.find((vt) => vt.value == result.key).fullName + ': ' + result.value">{{ voteTypes.find((vt) => vt.value == result.key).name }}: {{ $wallet.formatNumber(result.value) }}</b></small>
              </span>
            </div>
          </div>
        </div>
